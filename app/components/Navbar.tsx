...
const NavBar = () => {
      ...
      <div className="w-full">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {item.dropdown && (
              <ChevronDown
                className={`w-3 h-3 text-white transform transition-transform duration-200 ${isOpen ? "rotate-180" : ""
                  }`}
              />
            )}
          </div>
  ...
  return (
    <nav className="bg-[#00a651] py-2 w-full shadow-md font-sans border-x-2 border-green-800">
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden mx-auto space-y-1 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "block opacity-100 w-64" : "hidden opacity-0"
          }`}
      >
